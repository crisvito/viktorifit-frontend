...
export class Statistic implements OnInit {
  ...
  statCards = [
    { id: 'duration', title: 'Duration', value: '0 mins', icon: '/assets/statistic/duration.svg', change: '0%', changeText: 'From last month', isPositive: false },
    { id: 'weights', title: 'Weights', value: '0 kg', icon: '/assets/statistic/weights.svg', change: '0%', changeText: 'From last month', isPositive: false },
    { id: 'calories', title: 'Calories Burn', value: '0 cal', icon: '/assets/statistic/calories.svg', change: '0%', changeText: 'From last month', isPositive: false }
  ];
  ...
  public lineChartOptions: ChartOptions<'line'> = {
    ...
    scales: {
      x: { border: { display: false }, grid: { display: false }, ticks: { color: '#9ca3af', font: { family: "'Poppins', sans-serif", size: 11 }, padding: 15 } },
      y: { min: 0, grace: '20%', border: { display: false }, grid: { color: '#f3f4f6', tickLength: 0 }, ticks: { color: '#9ca3af', font: { family: "'Poppins', sans-serif", size: 11 }, maxTicksLimit: 6, padding: 20, callback: (value) => value + ' ' + this.getUnit() } }
    }
  };

  constructor() { this.generateYears(); }
  ...
  selectStat(id: string) { this.selectedStat = id; this.updateStatistics(); }
  ...
  applyPeriodSelection() { this.selectedMonthIndex = this.tempMonthIndex; this.selectedYear = this.tempYear; this.closePeriodModal(); this.updateStatistics(); }
